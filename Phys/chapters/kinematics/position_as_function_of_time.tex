...
\begin{tikzpicture}[scale=0.95]
    \draw[step=1, gray!25, very thin] (-0.5,-0.5) grid (3.5,4.5);
    \draw[->] (-0.5,0) -- (3.8,0) node[right] {$x$};
    \draw[->] (0,-0.5) -- (0,4.8) node[above] {$y$};
    \draw[domain=0:2.1, smooth, samples=80] plot (\x,{\x*\x}) node[right] {$\mathbf{r}(t)=(t,t^2)$};

    \draw[->, thick] (0,0) -- (1,1) node[midway, above left] {$\mathbf{r}(t_1)$};
    \draw[->, thick] (0,0) -- (2,4) node[midway, left] {$\mathbf{r}(t_2)$};
    ...
    \fill (1,1) circle (2pt);
    \fill (2,4) circle (2pt);
    \node[below right] at (1,1) {$t_1$};
    \node[above right] at (2,4) {$t_2$};
